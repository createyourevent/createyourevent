...
/**
 * REST controller for managing {@link org.createyourevent.app.domain.Address}.
 */
@RestController
@RequestMapping("/api")
public class AddressResource {
    ...
    public ResponseEntity<List<Address>> getAllAddresses(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("location-is-null".equals(filter)) {
            log.debug("REST request to get all Addresss where location is null");
            return new ResponseEntity<>(addressService.findAllWhereLocationIsNull(), HttpStatus.OK);
        }
