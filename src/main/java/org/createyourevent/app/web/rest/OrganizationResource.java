...
public class OrganizationResource {
    ...
    public ResponseEntity<List<Organization>> getAllOrganizations(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("restaurant-is-null".equals(filter)) {
            log.debug("REST request to get all Organizations where restaurant is null");
            return new ResponseEntity<>(organizationService.findAllWhereRestaurantIsNull(), HttpStatus.OK);
        }

        if ("hotel-is-null".equals(filter)) {
            log.debug("REST request to get all Organizations where hotel is null");
            return new ResponseEntity<>(organizationService.findAllWhereHotelIsNull(), HttpStatus.OK);
        }

        if ("club-is-null".equals(filter)) {
            log.debug("REST request to get all Organizations where club is null");
            return new ResponseEntity<>(organizationService.findAllWhereClubIsNull(), HttpStatus.OK);
        }

        if ("building-is-null".equals(filter)) {
            log.debug("REST request to get all Organizations where building is null");
            return new ResponseEntity<>(organizationService.findAllWhereBuildingIsNull(), HttpStatus.OK);
        }
