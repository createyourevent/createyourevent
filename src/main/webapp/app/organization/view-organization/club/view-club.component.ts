...
export class ViewClubComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.activatedRoute.data.subscribe(({ club }) => {
      ...
      this.generalService.findOrganizationReservationsByOrganizationId(this.club.organization.id).subscribe(r => {
      ...
      this.generalService.findImagesByOrganizationId(this.club.organization.id).subscribe(res => {
        this.images = res.body!;
      });
      this.generalService.findOrganizationLikeDislikeByOrganizationId(this.club.organization?.id!).subscribe(res => {
      ...
      this.generalService.findWidthAuthorities().subscribe(u => {
        ...
        this.generalService.findOrganizationLikeDislikeByOrganizationIdAndUserId(this.club.organization?.id!, this.user.id!).subscribe(res => {
        ...
        this.generalService.findOrganizationStarRatingsByOrganizationIdAndUserId(this.club.organization.id!, this.user.id!).subscribe(res => {
          if(res.body !== null) {
      ...
      this.generalService.findOrganizationCommentByOrganizationId(this.club.organization?.id!).subscribe(res => {
      ...
      const queryParam = club.organization.address!.replace(' ', '+');
      this.googleGeocoderService.getFromAddress(queryParam).subscribe((res: any) => {
        ...
        this.overlays = [
          new google.maps.Marker({position: posOrganization, title: this.club.organization.name}),
        ];
      });
      this.loading = false;
    });
  ...
  formatAddress(): void {
    const googleAddressArray = this.club.organization.address?.split(',');
  ...
  onCommented(comment: string): void {
    ...
    organizationComment.organization = this.club.organization;
  ...
  onChangesStars(val: any): void {
    ...
    organizationStarsRating.organization = this.club.organization;
  ...
  onLiked(comment: string): void {
    ...
    organizationLikeDislike.organization = this.club.organization;
