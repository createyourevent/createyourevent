...
export class BondComponent implements OnInit {
  ...
  trackId(index: number, item: IBond): number {
    return item.id!;
  }
  ...
  delete(bond: IBond): void {
    const modalRef = this.modalService.open(BondDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IBond[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
