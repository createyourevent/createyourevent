...
export class UserPointAssociationComponent implements OnInit {
  ...
  protected onSuccess(data: IUserPointAssociation[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/user-point-association'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.userPointAssociations = data ?? [];
