...
export class ServiceOfferComponent implements OnInit {
  ...
  trackId(index: number, item: IServiceOffer): number {
    return item.id!;
  }
  ...
  delete(serviceOffer: IServiceOffer): void {
    const modalRef = this.modalService.open(ServiceOfferDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.serviceOffer = serviceOffer;
  ...
  protected onSuccess(data: IServiceOffer[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
