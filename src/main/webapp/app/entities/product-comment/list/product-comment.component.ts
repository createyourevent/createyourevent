...
export class ProductCommentComponent implements OnInit {
  ...
  trackId(index: number, item: IProductComment): number {
    return item.id!;
  }

  delete(productComment: IProductComment): void {
    const modalRef = this.modalService.open(ProductCommentDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.productComment = productComment;
  ...
  protected onSuccess(data: IProductComment[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/product-comment'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
